...
def test_db_migration_resets_legacy_schema_to_snapshot_model(tmp_path: Path) -> None:
    ...
    with Database(db_file) as db:
        ...
        assert unique_columns == ["scan_id", "normalized_path"]
        assert int(db.conn.execute("PRAGMA user_version").fetchone()[0]) >= 17
...
def test_list_match_items_for_scan_excludes_failed_files_by_normalized_path() -> None:
    with Database(":memory:") as db:
        scan_id = db.create_scan(
            profile="balanced", roots=["D:/Videos"], extensions=["mp4"]
        )
        file_ok = db.upsert_file(
            path="D:/Videos/good.mp4",
            size=2,
            mtime_ns=2,
            ctime_ns=2,
            ext="mp4",
        ...
        db.save_video_meta(file_ok, meta)
        db.save_video_meta(file_bad, meta)
        db.save_fingerprint(file_ok, algo_version=ALGO_VERSION, hashes=[1, 2, 3])
        db.save_fingerprint(file_bad, algo_version=ALGO_VERSION, hashes=[4, 5, 6])
        ...
        assert [item.path for item in items] == ["D:/Videos/good.mp4"]
...
def test_cached_artifacts_can_include_audio_fingerprints() -> None:
    with Database(":memory:") as db:
        ...
        assert cached[path_key("D:/Videos/a.mp4")]["audio_fingerprint"] == "audio:abc"
...
def test_scan_batch_methods_roundtrip() -> None:
    with Database(":memory:") as db:
        ...
        normalized_a = path_key("D:/Videos/a.mp4")
        assert normalized_a in cached
        assert "meta" in cached[normalized_a]
        assert cached[normalized_a]["meta_probed_at"]
        assert "fingerprint" in cached[normalized_a]
        assert cached[normalized_a]["fingerprint_created_at"]
        assert cached[normalized_a]["fingerprint"]["algo_version"] == ALGO_VERSION
...
def test_files_case_variants_collapse_to_one_row_per_scan() -> None:
    with Database(":memory:") as db:
        scan_id = db.create_scan(
            profile="balanced",
            roots=["D:/Videos"],
        ...
        rows = db.conn.execute(
            "SELECT id, path, normalized_path FROM files WHERE scan_id = ?",
            (scan_id,),
        ).fetchall()
        ...
        assert str(rows[0]["path"]) == "d:/videos/a.mp4"
        assert str(rows[0]["normalized_path"]) == path_key("D:/Videos/A.mp4")


def test_load_duplicate_groups_preserves_group_and_item_order() -> None:
    with Database(":memory:") as db:
        scan_id = db.create_scan(
            profile="balanced",
            roots=["D:/Videos"],
        ...
        for file_id, meta in files:
            db.save_video_meta(file_id, meta)

        group_one = DuplicateGroup(
            scan_id=scan_id,
            profile="balanced",
            created_at="now",
            total_size_bytes=19,
        ...
        db.insert_duplicate_groups_batch(
            scan_id=scan_id,
            profile="balanced",
        ...
        assert [group.total_size_bytes for group in groups] == [19, 15]
        assert [item.path for item in groups[0].items] == [
            "D:/Videos/g1_best.mp4",
            "D:/Videos/g1_other.mp4",
        ]
        assert [item.path for item in groups[1].items] == [
            "D:/Videos/g2_big.mp4",
            "D:/Videos/g2_small.mp4",
        ]
...
def test_cached_artifacts_require_exact_stat_match_and_use_normalized_keys() -> None:
    with Database(":memory:") as db:
        scan_id = db.create_scan(
            profile="balanced",
            roots=["D:/Videos"],
        ...
        stale = db.load_cached_artifacts_batch(
            [{"path": "D:/Videos/a.mp4", "size": 123, "mtime_ns": 999}],
            algo_version=ALGO_VERSION,
        )

        assert set(cached) == {path_key("D:/Videos/A.mp4")}
        assert cached[path_key("D:/Videos/A.mp4")]["fingerprint"]["hashes"] == [1, 2, 3]
        assert stale == {}
